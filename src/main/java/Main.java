import com.chao.crawler.database.ConnectionManager;
...
public class Main {
	...
	public static void saveArticles(List<Article> articles) throws Exception {
		...
		PreparedStatement pstmt = ConnectionManager
				.getStatement("INSERT INTO blog_article (article_title,article_sign,"
						+ "article_type,article_url) VALUES (?,?,?,?)");
		for (Article article : articles) {
			System.out.println(article);
			pstmt.setString(1, article.getArticleTitle());
			pstmt.setString(2, article.getArticleSign());
			pstmt.setString(3, articles.get(0).getArticleType());
			pstmt.setString(4, articles.get(0).getArticleUrl());
	...
	public static Long getTotlePages() throws Exception {
		...
		Document document = Jsoup.parse(html);
		return Long.valueOf(
				document.select(".page_nav a").last().attr("href").split("=")[1]);
	}
